...
parser.add_argument('--retraction', default='exp', choices=['exp', 'norm'],
                    help='Try both, results vary up to ~1%')
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        optimizer.step(project_onto_tangent=args.project, projection_mode=args.mode)
    ...
    with torch.no_grad():
        ...
        print('Test:\t\t'
              ...
              'Acc@1 {top1.avg:.3f}\t'.format(
               batch_time=batch_time, loss=CE_losses, top1=top1))
...
def retract(layer, temp_layer):
    if args.mode == 'channelwise':
        ...
        if args.retraction == 'exp':
            ...
            R = old_x*math.cos(delta_norm) + (delta/delta_norm)*math.sin(delta_norm)
        elif args.retraction == 'norm':
            # Normalization
            norm = torch.norm(new_x, p='fro')
            R = new_x / norm
...
def layer_norm(layer):
    if args.mode == 'channelwise':
        l = layer.weight.view(layer.weight.shape[0], -1)
        norm = torch.mean(torch.norm(l, p=2, dim=1))
    elif args.mode == 'layerwise':
        norm = torch.norm(layer.weight, p='fro')
    return norm
